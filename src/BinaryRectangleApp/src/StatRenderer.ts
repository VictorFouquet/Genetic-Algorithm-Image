...
export class StatRenderer {
     ...
     */
    static render(
        bestFitness: number[], avgFitness:   number[], diversity:     number[], maxDiversity:  number,
        refImageSize: number,  output:       string,   mutationRate:  number,   executionTime: number
    ): void {
        const config: ChartConfiguration = StatRenderer.getConfig(
            bestFitness,
            avgFitness,
            diversity,
            refImageSize,
        ...
        // Creates chart
        let statChart = new ChartJSNodeCanvas(
            { width: 1200, height: 700, backgroundColour: 'rgb(10, 10, 10)' }
        );
        // Writes chart to file system
        fs.writeFileSync(
            output, 
            statChart.renderToBufferSync(config, 'image/png')
        );
    }
